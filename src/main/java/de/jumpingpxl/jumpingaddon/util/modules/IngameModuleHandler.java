...
public class IngameModuleHandler {
	...
	public IngameModuleHandler registerIngameModules() {
		category = new ModuleCategory("JumpingAddon", true, new ControlElement.IconData(Material.ENCHANTED_BOOK));//new ResourceLocation("jumpingaddon/textures/icon.png")));
		ModuleCategoryRegistry.loadCategory(category);
		serverSupportModule = (ServerSupportInGameModule) registerModule(new ServerSupportInGameModule(jumpingAddon));
		...
		gommeNickNameModule = (GommeNickNameInGameModule) registerModule(new GommeNickNameInGameModule(jumpingAddon));
		gommeReportsModule = (GommeReportsInGameModule) registerModule(new GommeReportsInGameModule(jumpingAddon));
		gommeTTTOnlineCoutModule = (GommeTTTOnlineCoutInGameModule) registerModule(new GommeTTTOnlineCoutInGameModule(jumpingAddon));
		gommeTTTRoleModule = (GommeTTTRoleInGameModule) registerModule(new GommeTTTRoleInGameModule(jumpingAddon));
		return this;
	}
