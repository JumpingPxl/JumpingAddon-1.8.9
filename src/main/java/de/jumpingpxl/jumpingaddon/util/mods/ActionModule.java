...
public class ActionModule {
	...
	public ActionModule loadActions() {
		if (jumpingAddon.getConfiguration().has("actions"))
			jumpingAddon.getConfiguration().get("actions").getAsJsonArray().forEach(jsonElement -> {
				...
				actionList.add(new Action(jumpingAddon, jsonObject.get("name").getAsString(), jsonObject.get("checkMessage").getAsString(), ActionCheckType.valueOf(jsonObject.get("checkType").getAsString()), jsonObject.get("response").getAsString(), jsonObject.get("delay").getAsInt(), jsonObject.get("enabled").getAsBoolean()));
			});
		return this;
	}
	...
	public static class Action {
		...
		public Action(JumpingAddon jumpingAddon, String name, String checkMessage, ActionCheckType checkType, String response, int delay, boolean enabled) {
			this.jumpingAddon = jumpingAddon;
			this.name = name;
		...
		private void respond(String message) {
			if (delay == 0)
				...
				new Task(() -> jumpingAddon.sendMessage(checkArguments(response, message))).delay(delay, TimeUnit.SECONDS);
		}
		...
		private void checkMessage(String message) {
			String toCheck = jumpingAddon.getStringUtils().stripColor(message);
			switch (checkType) {
				case STARTSWITH:
	...
	public enum ActionCheckType {
		STARTSWITH, ENDSWITH, SPLIT, CONTAINS, REGEX
	}
}
