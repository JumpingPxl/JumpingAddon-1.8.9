...
public class QuickShopCommand implements CommandExecutor, CommandSettings {
	...
	private final HashMap<Integer, Integer> traitorShop = new HashMap<>();
	private final HashMap<Integer, Integer> detectiveShop = new HashMap<>();
	...
	public boolean execute(CommandHandler.Command command, String label, String[] args) {
		...
		if (jumpingAddon.getConnection().getGameType() == null || !jumpingAddon.getConnection()
				...
				.getName()
				.equals("TTT")) {
			send(jumpingAddon, "quickShopNotInTTT", label);
		} else if (args.length == 0) {
			send(jumpingAddon, "commandUsage", label + " <shortcut>");
		} else {
			...
			i -= 1;
			if (i < 0 || i > 9) {
				send(jumpingAddon, "quickShopNotInRange");
				...
				send(jumpingAddon, "quickShopInnocent");
			} else {
				send(jumpingAddon, "quickShopNotIngame");
			}
		}
		return true;
	}
	...
	public Object[] getSettings(CommandHandler.Command command) {
		...
		traitor.add(new HeaderElement(
				"§bMiscellaneous §7» §bClient-Commands §7» §bQuickShop §7» §4Traitor-Shop"));
		detective.add(new HeaderElement(
				"§bMiscellaneous §7» §bClient-Commands §7» §bQuickShop §7» §9Detective-Shop"));
		...
		for (int i = 1; i <= 10; i++) {
			...
			traitor.add(new BetterNumberElement("Shortcut " + i + "\n §8» §4Traitor-Shop",
					new ControlElement.IconData(Material.IRON_SWORD), traitorShop.get(i), 0, 50, value -> {
			...
			detective.add(new BetterNumberElement("Shortcut " + i + "\n §8» §9Detective-Shop",
					new ControlElement.IconData(Material.BEACON), detectiveShop.get(i), 0, 50, value -> {
				traitorShop.put(integer, value);
				command.getConfiguration().set("detective" + integer, value);
				command.getConfiguration().save();
			}).getElement());
		}
		return new Object[]{new BetterListContainerElement("§4Traitor-Shop",
				new ControlElement.IconData(Material.IRON_SWORD)).addSettings(
				traitor.toArray(new Object[0])), new BetterListContainerElement("§9Detective-Shop",
				new ControlElement.IconData(Material.BEACON)).addSettings(
				detective.toArray(new Object[0]))};
	}
	...
	public void setSettings(CommandHandler.Command command, List<SettingValue> list,
		...
		for (int i = 1; i <= 10; i++) {
			traitorShop.put(i,
					configuration.has("traitor" + i) ? configuration.getAsInt("traitor" + i) : 0);
			detectiveShop.put(i,
					configuration.has("detective" + i) ? configuration.getAsInt("detective" + i) : 0);
		}
	}
}